      ...
      <el-table-column
        ...
        <template #default="scope">
          <el-radio-group v-model="scope.row.radio">
            ...
            <el-radio value="move" size="large">生成副本</el-radio>
...
async function moveObject(operationFileName, targetFileName, type) {
  let url
  if(type === 'folder'){
    url = "/minio/moveDir"
  } else {
...
const moveFileWithDiffName = async () => {
  ...
  for (let i = 0; i < fileObjects.value.length; i++) {
    ...
    if(row.type === 'file'){
      if(row.radio === 'move'){
        await moveObject(row.name, genNewFileName(row.name,allFileNames.value), 'file')
      } else if(row.radio === 'cover'){
        await moveObject(row.name, row.name, 'file')
      }
    } else {
      if(row.radio === 'move'){
        await moveObject(row.name, genNewFolderName(row.name,allFolderNames.value), 'folder')
      }
    }
  }
  closeLoading()
